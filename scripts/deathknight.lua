...
Define(raise_dead 46585)
    SpellInfo(raise_dead totem=1 cd=120 duration=60 gcd=0 offgcd=1)
    SpellRequire(raise_dead unusable set=1 enabled=(pet.exists()))
...
AddFunction BloodOssuaryThreshold
{
    ...
    # Keep Bone Shield stacks low when Dancing Rune Weapon (with Crimson Rune Weapon) or
    #    Abomination Limb is coming off cooldown.
...
AddFunction BloodDefaultMainActions
{
    ...
    # Blood Boil if a target does not have Blood Plague and (Venthyr) Swarming Mists is not active.
    if ((DebuffCountOnAny(blood_plague_debuff) < Enemies(tagged=1) or target.DebuffRefreshable(blood_plague_debuff)) and not BuffPresent(swarming_mist)) Spell(blood_boil)
    ...
    if (not BuffPresent(dancing_rune_weapon_buff) and BuffPresent(crimson_scourge_buff) and Enemies() >= 3) Spell(death_and_decay)
    ...
    if BuffStacks(bone_shield) >= 8 and BuffRemaining(bone_shield) >= 7.5
    {
        ...
        if (not BuffPresent(dancing_rune_weapon_buff) and BuffPresent(dnd_buff) and Enemies() >= 3 and RunicPowerDeficit() > 40) Spell(heart_strike)
    }
